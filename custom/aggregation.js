...
$(document).ready(function(){
		...
		$.getJSON('custom/principle1.json', function(json) {
			...
			for(var i=0; i<json.statement.length; i++) {
				...
				if (statement.critique.length == 1){var statementName = $('<div class="span6"><i class="icon-chevron-down"></i> ' + statement.name + ' (1 critique)</div>');}
				else {var statementName = $('<div class="span6"><i class="icon-chevron-down"></i> ' + statement.name + ' (' + statement.critique.length + ' critiques)</div>');}
				...
				for(var j=0; j<statement.critique.length; j++) {
					...
					if (critique.comment == null) {var comment = $('<div class="span9">(no comment)</div>');}
					else {var comment = $('<div class="span9">&quot;' + critique.comment + '&quot;</div>');}
				
					var expertiseScore = critique.expertise;
					avgExpTotal = avgExpTotal + expertiseScore;
					var expertiseType = "unknown";
				
					// If a person's expertise score if below 60, the person is a novice.
					// Else, if person's expertise score is below 75, the person is moderate.
					// Else, the person is an expert.
					if (expertiseScore < 60 && expertiseScore >= 0) {expertiseType = "novice";}
					else if (expertiseScore < 75 && expertiseScore >= 60) {expertiseType = "moderate";}
					else if (expertiseScore >= 75 && expertiseScore < 100) {expertiseType = "expert";}
				...
				var hue = 0;
				if(valence == 'positive'){hue = 91;}
				else if (valence == 'negative'){hue = 360;}
			...
			if (totalCritCount == 1){$('#title-approp').append('<div class="span8 principle-name">' + principleName + ' (1 total critique)</div><div class="span4 text-right"><a data-toggle="collapse" data-target=".approp-collapse" href="#">Collapse/Expand All</a></div>');}
			else {$('#title-approp').append('<div class="span8 principle-name">' + principleName + ' (' + totalCritCount + ' total critiques)</div><div class="span4 text-right"><a data-toggle="collapse" data-target=".approp-collapse" href="#">Collapse/Expand All</a></div>');}
		});
		...
		$.getJSON('custom/principle2.json', function(json) {
			...
			for(var i=0; i<json.statement.length; i++) {
				...
				$(collapseControlRow).append(statementTitleRow);
				$(table).append(collapseControlRow);
				var critiquesCollapse = $('<div id="' + collapseTarget + '" class="in collapse layout-collapse"></div>');
				var avgExpTotal = 0; // avgExpTotal keeps track of expertise of each critique within a statement
				...
				var col = Color('hsl('+ hue +', 50%, '+ lightness +'%)');
				$(percentage).css('background-color', col.toCSS());
				$(table).append(critiquesCollapse);
			...
			// Add name of principle plus collapse/expand all
			var principleName = json.name;
			if (totalCritCount == 1){$('#title-layout').append('<div class="span8 principle-name">' + principleName + ' (1 total critique)</div><div class="span4 text-right"><a data-toggle="collapse" data-target=".layout-collapse" href="#">Collapse/Expand All</a></div>');}
			else {$('#title-layout').append('<div class="span8 principle-name">' + principleName + ' (' + totalCritCount + ' total critiques)</div><div class="span4 text-right"><a data-toggle="collapse" data-target=".layout-collapse" href="#">Collapse/Expand All</a></div>');}
		});
